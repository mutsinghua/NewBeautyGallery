...
/**
 * 滑动画廊2代,采用Adapter获取数据
 * 
 * @author blackiedeng
 * 
 */
public class SlideGallery2 extends AdapterView<BaseAdapter> {
	 ...
	 */
	private Queue<View> mViewsRecycleBin = new LinkedList<View>();
		...
		public boolean onScroll(MotionEvent e1, MotionEvent e2, float distanceX, float distanceY) {
			...
			if (mAdapter != null) {
				...
				if ((distance < 0 && mLeftMostResistance && mCurrentSelection == 0)
						|| (distance > 0 && mRightMostResistance && mCurrentSelection == mAdapter.getCount() - 1)) {
					// 滑到边缘了,增加阻力
					distance *= 0.2;
				} else {
					distance *= 0.5;
			...
			for (int i = 0; i < getChildCount(); i++) {
				...
				if (isEventWithinView(e, child)) {
					if (mOnItemClicked != null) {
						...
						mOnItemClicked.onItemClick(SlideGallery2.this, child, position, mAdapter.getItemId(position));
					}
					break;
				}
			}
		...
		if (mCurrentSelectedView != null) {
			...
			if (nextIndex >= 0 && nextIndex < getChildCount()) {
				...
				if (nextView != null) {
					...
					mCurrentSelection++;
					if (mListener != null) {
						mListener.onItemSelected(this, mCurrentSelectedView, mCurrentSelection);
					}
				}
			}
		}
	...
	private void startScroll(int offset) {
		...
		mNextLeft = 0;
		mCurrentLeft = 0;
		mScroller.startScroll(mNextLeft, 0, (int) offset, 0, SCROLL_DURATION);
	...
	private void resetLayout(int w, int h) {
		if (mCurrentSelectedView != null) {
			...
			mCurrentSelectedView.measure(MeasureSpec.makeMeasureSpec(w, MeasureSpec.EXACTLY), MeasureSpec.makeMeasureSpec(h, MeasureSpec.EXACTLY));
	...
	public SlideGallery2(Context context, AttributeSet attrs) {
		this(context, attrs, 0);
	}

	public SlideGallery2(Context context, AttributeSet attrs, int defStyle) {
	...
	private void recycleViewsOutOfBounds(int deltaLeft) {
		...
		while (child != null && child.getRight() + deltaLeft <= 0 - getWidth()) {
			...
			TouchableImageView imgView = (TouchableImageView) child.findViewById(R.id.touch_img);
			if (imgView != null) {
				if ((Integer) imgView.getTag(R.id.TAG_VIEW_FLAG) == 1) {
					imgView.getBitmap().recycle();
	...
	protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
		...
		if (heightMode == MeasureSpec.AT_MOST) {
			heightSize = getPaddingTop() + getPaddingBottom() + mMaxChildHeight + getHorizontalScrollbarHeight();
		}
